...
import { Event, getUser, kv, User, Ticket } from "@/utils/db/kv.ts";
...
export const handler: Handlers = {
	async POST(req, _ctx) {
		...
		const [event, ticket] = await kv.getMany<[Event, Ticket]>([["event", eventID], ["ticket", eventID, showTimeID, ticketID]]);
